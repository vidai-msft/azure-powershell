...
({} | Select-Object "Module", "TotalCommands", "TotalTestedCommands", "CommandsCoverage", "TotalParameterSets", "TotalTestedParameterSets", "ParameterSetsCoverage", "TotalParameters", "TotalTestedParameters", "ParametersCoverage" | ConvertTo-Csv -NoTypeInformation)[0] | Out-File -LiteralPath $cvgReportCsv -Encoding utf8 -Force
...
foreach ($moduleName in $allModules) {
    ...
    $moduleCsvReport = [PSCustomObject]@{
        ...
        TotalTestedCommands      = $totalExecutedCmdletsCount
        CommandsCoverage         = $cvgCmdlets
        TotalParameterSets       = $totalParameterSetsCount
        TotalTestedParameterSets = $totalExecutedParameterSetsCount
        ParameterSetsCoverage    = $cvgParameterSets
        TotalParameters          = $totalParametersCount
        TotalTestedParameters    = $totalParameterSetsCount
...
$overallCsvReport = [PSCustomObject]@{
    ...
    TotalTestedCommands      = $overallExecutedCmdletsCount
